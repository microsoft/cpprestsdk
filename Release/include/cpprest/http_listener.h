            ...
            namespace listener
            {
                ...
                public:
                    ...
                    http_listener_config(const http_listener_config &other)
                        : m_timeout(other.m_timeout), m_backlog(other.m_backlog)
                    ...
                    http_listener_config(http_listener_config &&other)
                        : m_timeout(std::move(other.m_timeout)), m_backlog(std::move(other.m_backlog))
#if !defined(_WIN32) || defined(CPPREST_FORCE_HTTP_LISTENER_ASIO)
                          ,
...
#include <map>
#endif
#if defined(CASA_JSON_H)
#include <cpprest/json.h>
#endif
                class rest_api_router
                {
                private:
                    using action_pointer = std::function<web::json::value(const web::http::http_request &)>;
                    static std::map<utility::string_t, action_pointer> routes_container;
                    static void add(utility::string_t endpoint, action_pointer actionPtr)
                    {
                        routes_container.insert(std::pair<utility::string_t, action_pointer>(endpoint, actionPtr));
                    }
                    static web::json::value execute(utility::string_t endpoint, const web::http::http_request &request)
                    {
                        std::map<utility::string_t, action_pointer>::iterator it;
                        it = rest_api_router::routes_container.find(endpoint);
                        if (it == rest_api_router::routes_container.end())
                ...
                public:
                    ...
                    {
                    public:
                        using action_pointer = web::json::value (C::*)(const web::http::http_request &);
                        register_route(utility::string_t endpoint, action_pointer action)
                        {
                            C *c = new C;
                            rest_api_router::add(endpoint, std::bind(action, c, std::placeholders::_1));
                            delete (c);
                        }
                    };
                    static const void route_handler(const web::http::http_request &request)
                    {
                        web::json::value result;
                        result = web::http::experimental::listener::rest_api_router::execute(request.relative_uri().to_string(), request);
                        if (result != web::json::value::null())
                            request.reply(web::http::status_codes::OK, result);
                        else
                            request.reply(web::http::status_codes::NotFound);
                    }
                };
                std::map<utility::string_t, rest_api_router::action_pointer> rest_api_router::routes_container;
                /// <summary>
                ...
                public:
                    ...
                    /// <summary>
                    /// Add support for a rest apit routing.
                    /// </summary>
                    /// <param name="route_config">Function object to be called onc time for add all users endpoints.</param>
                    void support_rest_api_routing(const std::function<void()> &route_config)
                    {
                        route_config();
                        m_impl->m_all_requests = &rest_api_router::route_handler;
                    }
