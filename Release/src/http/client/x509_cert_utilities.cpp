...
bool verify_X509_cert_chain(const std::vector<std::string>& certChain, const std::string& hostname)
{
    ...
    cert.raw = CertCreateCertificateContext(
    ...
    if (!CertGetCertificateChain(
            ...
            CERT_CHAIN_REVOCATION_CHECK_CHAIN, nullptr, &chainContext.raw))
    {
        return false;
    }
    ...
    if (!CertVerifyCertificateChainPolicy(
            CERT_CHAIN_POLICY_SSL, chainContext.raw, &policyPara, &policyStatus))
    {
        return false;
    }
