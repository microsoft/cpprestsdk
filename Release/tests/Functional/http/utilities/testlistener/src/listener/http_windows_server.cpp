...
unsigned long http_windows_server::register_listener(_In_ http_listener_interface *pListener)
{
    ...
    utility::string_t host_uri = web::http::uri::decode(u.to_string());
...
{
    // Windows HTTP Server API will not accept a uri with an empty path, it must have a '/'.
    const uri listener_uri = pListener->uri();
...
void http_windows_server::connection_closed_completion::http_io_completion(DWORD error_code, DWORD)
{
    ...
    {
        report_error(logging::LOG_ERROR, error_code, U("Error listening for HTTP Server connection close"));
    }
    else
    {
...
void http_windows_server::receive_requests()
{
    ...
    for(;;)
    {
        ...
        {
            // Process the request
            http_request msg = http_request::_create_request(std::unique_ptr<_http_server_context>(new windows_request_context(pConnection)));
...
pplx::task<void> http_windows_server::respond(http_response response)
{
