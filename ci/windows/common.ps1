...
function Unpack-Dependency {
    Param ([string]$dep_archive)

    if (-not (Test-Path $env:DVAULT_DEPENDENCIES\$dep_archive)) {
        throw "didn't find $dep_archive in $env:DVAULT_DEPENDENCIES"
    }

    Expand-Archive -Path $env:DVAULT_DEPENDENCIES\$dep_archive `
                   -DestinationPath $DEPENDENCIES_DIR          `
                   -Force
}

function Unpack-Dependencies {
    Unpack-Dependency gf_complete.zip
    Unpack-Dependency Jerasure.zip
    Unpack-Dependency botand.zip
    Unpack-Dependency botan.zip
    Unpack-Dependency cpprest.zip
}

function Recreate-Build-Location {
...
function Recreate-Result-Location {
    if (Test-Path $TESTRESULTS_DIR) {
        Remove-Item -Recurse -Force $TESTRESULTS_DIR
    }

    New-Item -ItemType directory -Path $TESTRESULTS_DIR
}

function Is-Master-Branch {
    return ($env:DVAULT_BRANCH_NAME -eq "master") -or `
           ($env:DVAULT_BRANCH_NAME -eq "origin/master")
}

function Copy-CppRest {
    Copy-Item $DEPENDENCIES_DIR\usr\local\lib\cpprest140_2_9.dll $BUILD_DIR
    Copy-Item $DEPENDENCIES_DIR\usr\local\lib\cpprest140d_2_9.dll $BUILD_DIR
}
